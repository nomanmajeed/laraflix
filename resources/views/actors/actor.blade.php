...
@section('content')
    <div class="movie-info border-b border-gray-800">
        <div class="container mx-auto px-4 py-16 flex flex-col md:flex-row">
            <div class="flex-none">
                <img src="{{ 'https://image.tmdb.org/t/p/w300' . $actor['profile_path'] }}" alt="profile image"
                    class="w-76">
                <ul class="flex items-center mt-4">

                    @if ($social['facebook_id'])
                        <li>
                        <a href="{{ 'https://facebook.com/'.$social['facebook_id'] }}" title="Facebook" target="_blank">
                    ...
                    @if ($social['instagram_id'])
                        <li class="ml-6">
                            <a href="{{ 'https://instagram.com/'.$social['instagram_id'] }}" title="Instagram" target="_blank">
                                <svg class="fill-current text-gray-400 hover:text-white w-6" viewBox="0 0 448 512">
                                    <path
            ...
            <div class="md:ml-24">
                ...
                <div class="flex flex-wrap items-center text-gray-400 text-sm">
                    ...
                    <span class="ml-2">{{ \Carbon\Carbon::parse($actor['birthday'])->format('M d, Y') }}
                        ({{ \Carbon\Carbon::parse($actor['birthday'])->age }} years old) in
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-8">
                    @foreach ($topFiveMovies as $topMovie)
                        @if ($topMovie['media_type'] == 'movie')
                            <div class="mt-4">
                                <a href="{{ route('movies.show', $topMovie['id']) }}">
                                    @if ($topMovie['poster_path'])
                                        <img
                                        src="{{ 'https://image.tmdb.org/t/p/w300' . $topMovie['poster_path'] }}"
                                        alt="poster" class="hover:opacity-75 transition ease-in-out duration-150">
                                    @else
                                        <img
                                        src="{{ 'https://ui-avatars.com/api/?size=512&name='.$topMovie['title'] }}"
                                        alt="poster" class="hover:opacity-75 transition ease-in-out duration-150">
                                    @endif
                                    
                                    
                                    </a>
                                <a href="{{ route('movies.show', $topMovie['id']) }}" class="text-sm leading-normal block text-gray-400 hover:text-white mt-1">{{ $topMovie['title'] }}</a>
                            </div>
                        @else
                            <div class="mt-4">
                                <a href="{{ route('tv.show', $topMovie['id']) }}"><img
                                        src="{{ 'https://image.tmdb.org/t/p/w300' . $topMovie['poster_path'] }}"
                                        alt="poster" class="hover:opacity-75 transition ease-in-out duration-150"></a>
                                <a href="{{ route('tv.show', $topMovie['id']) }}" class="text-sm leading-normal block text-gray-400 hover:text-white mt-1">{{ $topMovie['title'] }}</a>
                            </div>
                        @endif
